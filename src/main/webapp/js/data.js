...
'220625':['江源县','220600','jiang yuan xian'],
...
'610403':['杨凌区','610400','yang ling qu'],
...
'820101':['花地玛堂区','820100','hua ma tang qu'],
'820102':['圣安多尼堂区','820100','sheng an duo ni qu'],
'820103':['大堂区','820100','da tang qu'],
'820104':['望德堂区','820100','wang de tang qu'],
'820105':['风顺堂区','820100','feng shun tang qu'],
'820200':['离岛','820000','li dao'],
'820201':['嘉模堂区','820200','jia mo tang qu'],
'820202':['圣方济各堂区','820200','sheng fang ji ge tang qu'],
'820300':['无堂区','820000','wu tang qu'],
'820301':['路氹城','820300','lu dang cheng'],
'990000':['海外','1','hai wai'],
...
/**
 * 此类中提供了4个方法
 * @param read 实例化省市县三级联动对象
 * @param findByName 通过名称去寻找 
 * @param findBySpell 通过拼音去寻找
 * @param findByString 先通过名称去查找，如果不存在就用拼音去查找
 * @returns {Pan}
 */
function Pan(){
	var value="请选择...";
	/**
	 * 设置默认的select值
	 * @param optionValue 必须要再read之前进行调用
	 */
	this.setDefaultOption=function(optionValue){
		value=optionValue;
	};
	 ...
	 * @param a 区/县
	 */
	this.read=function(p,c,a){		//去除县
	//this.read=function(p,c,a){
		pObj=$$(p);
		cObj=$$(c);
		aObj=$$(a);
		
		pObj.options.add(new Option(value,0));
		cObj.options.add(new Option(value,0));
		aObj.options.add(new Option(value,0));
		//为每一级添加初始值
		for(var item in tdist){
			
			var obj=tdist[item];
			if (obj[1] == "1") {
				pObj.options.add(new Option(obj[0],obj[0]));
			}
         ...
         * 通过名称去寻找符合条件的数据，模糊查询
         * @param 要进行查找的值
         * @returns 返回的是一个Array数组
         */
        this.findByName = function (string) {
         ...
         * @returns 返回的是一个Array数组
         */
        this.findBySpell=function(spell){
              ...
              for (var item in tdist) {
                  var arr = tdist[item];
                  if (arr[2].replace(/\s/gi,"").indexOf(spell) >= 0) {
                      array[index++] = arr[0]+arr[2].replace(/\s/gi,"");
                  }
              }
              return array;
         ...
         * @returns 返回的是一个Array数组
         */
        this.findByString=function(string){
        	var array=this.findByName(string);
        	
        	if(array.length<=0){
        		array=this.findBySpell(string);
        	}
 ...
 */
function event(){
	pObj.onchange=provinceChange;
	cObj.onchange=cityChange;
}

//省

function provinceChange() {
	cObj.options.length=1;
	aObj.options.length = 1;
	setArray(findByName(pObj.value), cObj);
}
//城市
function cityChange(){
	aObj.options.length=1;
	
	setArray(findByName(cObj.value), aObj);

	//如果没有任何选项 就添加一个其他
	if(aObj.options.length==1){
		aObj.options.add(new Option('其他','其他'));
	}
}

function setArray(baseid, sub) {

	for ( var item in tdist) {
		var obj=tdist[item];
		if (baseid == obj[1] ) {
			 sub.options.add(new Option(obj[0],obj[0]));
		 }
