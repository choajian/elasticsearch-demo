$(document).ready(function(){
	...
	if ('' != $('#noticeTypes').val())
	{
		var noticeTypes = $('#noticeTypes').val().split(',');
		for (var i = 0; i < noticeTypes.length; i++) 
		{
			$("input[name='xmStyle'][value='" + noticeTypes[i] + "']").attr("checked", "checked");
		}
	}
	if ('' != $('#areas').val())
	{
		...
		{
			$("input[name='voteitem'][value='" + areas[i] + "']").attr("checked", "checked");
		}
		$('#xz').html($('#areas').val().replace(/,/g," "));
	}
});
...
function submitRss() {
   ...
   //收集订阅详情
    ...
    var len = $("input[name='searchStyle']:checked").length;
    if (0 == len)
    {
    	alert('请至少选择一种检索种类！');
    	return false;
    ...
    {
    	$('#projectFlag').val($("input[name='searchStyle']:checked").val());
    }
