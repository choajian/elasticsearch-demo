	(function($) {
		...
		$.suggest = function(input, options) {
			...
			$input.blur(function() {
				...
				/**
				 * add 级联清空省级数据
				 * @author chaojian123@sina.com
				 */
				if ('' == $(this).val()) {
					$('#'+options.province).val('');
					$('#'+options.provinceNum).val('');
					/////注释2014-9-18
					//$('#'+options.visarea).val('');
					//$('#'+options.visarea).next().val('');
					//$('#'+options.visarea).prev().val('');
				}
			});
			...
			function resetPosition() {
				...
				$results.css({
					top: (offset.top + input.offsetHeight - 75) + 'px',
					//update chaojian left: (offset.left-110) + 'px'
					left: (offset.left) + 'px'
				});
			}
			...
			function displayItems(items) {
				...
				if (items=='') {//热门城市遍历
					...
					if (html == '') {
						...
						if(cityNum==1){
							$input.val(city[1]);
							var province = getProvince(city[1]);
							$('#'+options.provinceNum).val(province.substring(0, province.indexOf(',')));
							$('#'+options.province).val(province.substring(province.indexOf(',')+1, province.length));
							
							var $visarea=$('#'+options.visarea);
							var visareaText=$visarea.val();
							$visarea.css('color','black');
							if('' ==visareaText  || '中文/拼音'== visareaText){
								$visarea.val($input.val());
								$visarea.next().val($input.next().val());
								$visarea.prev().val($input.prev().val());
							}
						}
			...
			function selectCurrentResult() {
				...
				if ($currentResult) {
					 ...
					 * @author chaojian123@sina.com
					 */
					var province = getProvince($input.val());
					$('#'+options.provinceNum).val(province.substring(0, province.indexOf(',')));
					$('#'+options.province).val(province.substring(province.indexOf(',')+1, province.length));
					/**
					 * add 级联其它个地区文本框
